...
const AwardSection = ({ register, errors, control }: AuthFormProps) => {
    ...
    <div className="pt-4 space-y-4">
      ...
      {fields.map((field, index) => (
        <div key={field.id} className="border p-4 rounded space-y-2">
          <FormInput
            label="Başlık"
            error={errors?.awards?.[index]?.title}
          ...
          <FormInput
            label="Yıl"
            error={errors?.awards?.[index]?.year}
          ...
          <FormInput
            label="Açıklama"
            error={errors?.awards?.[index]?.description}
          ...
          <FormInput
            label="Organizasyon"
            error={errors?.awards?.[index]?.organization}
